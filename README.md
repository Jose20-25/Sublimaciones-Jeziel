# ERP Sublimaciones Jeziel

Sistema ERP web profesional para la gestión de negocios de sublimación y personalizados.

## Características principales

- **Dashboard** con métricas y gráficas en tiempo real.
- **Gestión de Inventario**: productos, stock, precios, búsqueda y exportación.
- **Ventas**: registro, edición, eliminación, generación de factura y tique en PDF, integración con clientes e inventario.
- **Clientes**: alta, edición, eliminación y búsqueda de clientes.
- **Compras**: registro de compras, actualización automática de inventario, integración con finanzas.
- **Finanzas**: ingresos, egresos, balance automático, conexión con compras.
- **Reportes**: filtros por fecha y tipo, exportación a PDF/Excel, gráficas de ventas, compras y finanzas.
- **Interfaz profesional y responsiva**: Bootstrap 5, Chart.js, jsPDF, SheetJS.
- **Persistencia local**: todos los datos se guardan en el navegador (localStorage).

## Archivos principales

- `index.html`: Dashboard principal y navegación.
- `inventario.html`: Gestión de inventario.
- `ventas.html`: Gestión de ventas y generación de documentos.
- `clientes.html`: Gestión de clientes.
- `compras.html`: Registro de compras.
- `finanzas.html`: Módulo de finanzas.
- `reportes.html`: Reportes y estadísticas.
- `logo/logo.jpg`: Logo de la empresa.

## Instalación y uso

1. Descarga o clona este repositorio.
2. Abre el archivo `index.html` en tu navegador.
3. Navega entre los módulos usando el menú lateral.
4. Todos los datos se almacenan localmente en tu navegador.

## Requisitos
- Navegador moderno (Chrome, Edge, Firefox, Opera, Safari)
- No requiere servidor ni base de datos externa.

## Créditos
- Desarrollado para Sublimaciones Jeziel.
- UI: Bootstrap 5, Bootstrap Icons, Google Fonts (Montserrat)
- Gráficas: Chart.js
- PDF: jsPDF
- Excel: SheetJS (xlsx)

---

© 2025 Sublimaciones Jeziel. Todos los derechos reservados.
